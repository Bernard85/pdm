	...
	@Inject public void Studies(Composite parent) throws Throwable{
		...
		tStudies.setContentProvider(new ITreeContentProvider() {
			...
			public Object[] getChildren(Object o) {
				...
				for (int c=0;c<node.getChildNodes().getLength();c++) {
					...
					Element element = getChild((Element) node2);

					elements.add(element);
				}
				return elements.toArray(new Element[elements.size()]);
			}
			Element getChild(Element element) {
				Element rightAlias = (Element) element.getUserData(HUB.RIGHT_ALIAS);
				return (rightAlias!=null)?rightAlias:element;
			}

			public Object getParent(Object o) {
				Element element = (Element) o;
				Element leftAlias = (Element) element.getUserData(HUB.LEFT_ALIAS);
				return (leftAlias==null)?element.getParentNode():leftAlias.getParentNode();
			}
		});
		...
		tStudies.setLabelProvider(new LabelProvider() {
			public Image getImage(Object o) { 
				...
				/*if (element.hasAttribute("open") && Boolean.getBoolean(element.getAttribute("open"))) imageID+="_open";*/
			...
			public String getText(Object o) {
				...
				if (element.hasAttribute("ID")) text="["+element.getAttribute("ID")+"] "+text;
				return text;
			}
		});
