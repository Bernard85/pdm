...
class Jrn1 implements IExo{
	...
	@PostConstruct public void postConstruct (Composite parent)   {
		...
		jrnViewer1=new JrnViewer1(parent);
		fd =new FormData();
		fd.left=new FormAttachment(0);
		...
		jrnViewer1.setLayoutData(fd);
	    long start = System.currentTimeMillis();
	    
		jrnViewer1.setColumns(FILE_NAME+".col");
		jrnViewer1.setFields(FILE_NAME+".fmt");
		jrnViewer1.setData(FILE_NAME+".csv");

        long end = System.currentTimeMillis();
	    float sec = (end - start) / 1000F; 
	    System.out.println(sec + " seconds");

		jrnViewer1.setMessageBox(new IMessageBox() {
			@Override
		...
		jrnViewer1.setModifierNotifier(new IModifierNotifier() {
			@Override
	...
	@Persist  void saveIt() {
		jrnViewer1.saveIt(FILE_NAME+".col");
	};
}
